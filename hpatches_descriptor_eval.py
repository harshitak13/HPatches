...
========================================
HPatches = Homography Patches  (Yi et al., ECCV 2016 benchmark).
Evaluates SIFT, SURF (if available), ORB, BRISK, LATCH, and LIFT descriptors on the
HPatches dataset across three standard tasks:
  1. Patch Verification       — does a pair of patches show the same point?
  2. Image Matching           — match keypoints between a reference and warped image
  3. Patch Retrieval          — rank a gallery of patches by similarity to a query

Two dataset modes are supported
--------------------------------
Mode A – Full-image sequences (original HPatches format)
  hpatches-sequences-release/
    v_*/          <- viewpoint change sequences
    i_*/          <- illumination change sequences
      1.ppm       <- reference image
      2.ppm ... 6.ppm  <- target images
      H_1_2 ... H_1_6  <- ground-truth 3×3 homography matrices
  All three tasks (MMA + Verification + Retrieval) use full images.

Mode B – Pre-extracted homography patch files (patch mode)
  hpatches-sequences-release/
    v_*/
    i_*/
      ref.png     <- reference patches  (N*65 × 65 grayscale stack)
      e1.png … e5.png  <- easy positive patches
      h1.png … h5.png  <- hard positive patches
      t1.png … t5.png  <- tough positive patches
  Patch Verification and Retrieval use the pre-extracted patches.
  Image Matching (MMA) is skipped (no full images / homographies).
  Auto-detected when ref.png is present; override with --patch_mode.

Usage
-----
  # Full-image mode (auto-detected):
  python hpatches_descriptor_eval.py --hpatches_dir ./hpatches-sequences-release

  # Force patch-file mode:
  python hpatches_descriptor_eval.py --hpatches_dir ./hpatches-sequences-release --patch_mode

  # Generate synthetic demo patches (no dataset needed):
  python hpatches_descriptor_eval.py --demo

  # Supply pre-trained LIFT weights (optional; random init used otherwise):
...
------------
  ...
  pip install torch torchvision   # optional – required for LIFT descriptor
"""
